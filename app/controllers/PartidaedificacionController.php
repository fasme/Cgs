<?php
class PartidaedificacionController extends BaseController {

public function mostrar(){

	//$partidas = Partida::all();
	$partidas = Partida::where('proyecto_id','=',Session::get("proyecto")->id)->get();
	return View::make("partidas.listaEdificacion", array("partidas"=>$partidas));
...
public function nuevo(){
	...
	$obras = Obra::where('proyecto_id',"=",Session::get("proyecto")->id)->get()->lists("nombre","id");
	  $obras = $obras0 + $obras;
	$selected = array();
	return View::make("partidas.crearEdificacion")->with("obras",$obras)->with("categorias",$categorias)->with("partida",$partida);
	//return "ho";
}
...
public function nuevo2(){
	...
	if($partida->isValid($data,""))
	{
		$partida->fill($data);
		$partida->save();
		return Redirect::to('partidas/nuevoEdificacion');
	}
	else
	{
		return Redirect::to('partidas/nuevoEdificacion')->withInput()->withErrors($partida->errors);
	}
